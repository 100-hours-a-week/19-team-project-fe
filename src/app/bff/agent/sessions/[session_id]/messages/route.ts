...
import { getAgentSessionMessages } from '@/features/agent.server';
...
export async function GET(req: Request, context: { params: Promise<{ session_id?: string }> }) {
  try {
    ...
    const data = await getAgentSessionMessages(sessionId, accessToken);
    ...
    console.error('[Agent Session Messages Error]', error);
    const response: ApiResponse<null> = {
      code: 'AGENT_SESSION_MESSAGES_FAILED',
      message: 'AGENT_SESSION_MESSAGES_FAILED',
      data: null,
    };
