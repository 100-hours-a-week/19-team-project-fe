...
### 현재 상황
- 전역 proxy 가드로 정적 에셋까지 차단되어 `/assets/lanyard/card.glb` 로드 실패
- 프록시 제거/정리 과정에서 `chat/[chatId]` 직접 접근 차단 로직이 누락됨

### 목표
- 전역 proxy 제거
- `chat/[chatId]` 경로만 서버 가드로 차단
- 내부 이동은 정상 동작하고, 직접 접근은 `/`로 리다이렉트 + 경고 토스트

## 작업 내용
- [x] 전역 proxy 가드 제거 (`src/proxy.ts` 삭제)
- [x] `chat/[chatId]` 서버 가드 복구 (`getChatDetail` 실패 시 `/?guard=invalid` 리다이렉트)
- [x] 홈 경고 토스트 유지 (`HomeGuardToast`)

---

## AT 재발급 후 1회 재시도 로직 정리

### 목표
- 401 발생 시 RT 재발급 성공한 경우에만 **원 요청을 1회 재시도**
- 재시도 시 **최신 AT로 Authorization 헤더 갱신**
- 토큰 재발급 요청 자체는 재시도 대상에서 제외

### 적용 내용
- `apiFetch`에 401 재시도 로직 추가
  - RT 재발급 성공 시에만 재요청
  - 재요청은 1회만 수행(무한 루프 방지)
- 재시도 시 `Authorization` 헤더가 있으면 **쿠키의 최신 AT로 교체**
- `/bff/auth/tokens` 요청은 `retryOnUnauthorized: false`로 재시도 제외

### 변경 파일
- `src/shared/api/apiFetch.ts`
  - 401 → refresh 성공 시 1회 재시도
  - 재시도 시 최신 AT로 Authorization 헤더 갱신
- `src/shared/api/refreshTokens.client.ts`
  - 재발급 요청에 `retryOnUnauthorized: false` 적용
